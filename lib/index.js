...
function pdf( x, options ) {
	...
	if ( isNumber( x ) ) {
		return pdf1( x, opts.mu, opts.beta );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return pdf5( out, x, opts.mu, opts.beta );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.mu, opts.beta );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.mu, opts.beta, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.mu, opts.beta, opts.accessor );
		}
		return pdf2( out, x, opts.mu, opts.beta );
	}
	return NaN;
} // end FUNCTION pdf()
